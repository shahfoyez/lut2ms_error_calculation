            ...
            <div class="card mb-5 mb-xl-8">
                ...
                <div class="card-header border-0 pt-5">
                    ...
                    <div class="card-toolbar">
                        <?php
                            $formAction = url('/trip/history/filter');
                            $refreshAction = url('/trip/history');
                        ?>
                            ...
                            <tbody>
                                @if($trips && $trips->count()>0)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="d-flex justify-content-start flex-column">
                                                    <a href="{{ url('/vehicle/edit/'.$trip->vehicle->id) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{ $trip->vehicle->codeName }}</a>
                                                    <span class="text-muted fw-bold text-muted d-block fs-7">License: {{ $trip->vehicle->license ? $trip->vehicle->license : "No Data" }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <p class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $trip->vehicle->capacity }}</p>
                                            <span class="text-muted fw-bold text-muted d-block fs-7">Route: {{ $trip->rout->route }}</span>
                                        </td>
                                        <td>
                                            <a href="{{ url('/employee/employeeEdit/'.$trip->employee->id) }}" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $trip->employee->name }}</a>
                                        </td>
                                        <td>
                                            <p class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $trip->from }}</p>
                                        </td>
                                        <td>
                                            <p class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $trip->dest }}</p>
                                        </td>
                                        <td>
                                            <p class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $trip->start->format('d M Y') }}</p>
                                            <span class="text-muted fw-bold text-muted d-block fs-7">{{ $trip->start->format('h:i A') }}</span>
                                        </td>
                                        <td>
                                            <p class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $trip->end->format('d M Y') }}</p>
                                            <span class="text-muted fw-bold text-muted d-block fs-7">{{ $trip->end->format('h:i A') }}</span>
                                        </td>
                                        <td>
                                            <p class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ $trip->tripDistance->distance ?? 'No Data' }}</p>
                                        </td>
                                        <td>
                                            @php
                                                if($trip->status == 1){
                                        ...
                                        <td>
                                            <a href="#" data-bs-toggle="modal" data-bs-target="#delete_Modal" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm cd_modal" data-item="{{ url('/requisition/delete/'.$trip->id) }}" >
