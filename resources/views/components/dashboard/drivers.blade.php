...
@section('mapScript')
    ...
    <script>
        ...
        function addMarker(data) {
            ...
            // Get the scaled size of the marker icon
            var markerScaledSize = marker.getIcon().scaledSize;

            // Calculate the radius based on the average of the width and height of the scaled size
            var circleRadius = Math.max(markerScaledSize.width, markerScaledSize.height) / 2;
            var circle = new google.maps.Circle({
                map: map,
                radius: circleRadius, // Circle radius in meters
                ...
                success: function(allData) {
                    ...
                    if(wlsLength > 0){
                        ...
                        for (const key in withLocationShow) {
                            ...
                            // i++;
                            animateMarker(markers[i], { lat: newlat, lng: newlong });
                            i++;
                        }
                        ...
                        // smoothPanTo(map, bounds.getCenter(), 12);
                        smoothPanTo(map, bounds.getCenter());
                    }else{
                        noVehicle();
                    }
                }
        ...
        // Function to smoothly move markers
        function animateMarker(marker, newLocation) {
            ...
            if (newLocation instanceof google.maps.LatLng) {
                deltaLat = (newLocation.lat() - oldLocation.lat()) / numSteps;
                deltaLng = (newLocation.lng() - oldLocation.lng()) / numSteps;
            } else if (typeof newLocation === 'object' && 'lat' in newLocation && 'lng' in newLocation) {
                deltaLat = (newLocation.lat - oldLocation.lat()) / numSteps;
                deltaLng = (newLocation.lng - oldLocation.lng()) / numSteps;
            } else {
                console.error('Invalid newLocation format');
                return;
            }

            function moveMarker() {
                oldLocation = new google.maps.LatLng(
                    oldLocation.lat() + deltaLat,
                    oldLocation.lng() + deltaLng
                );
        ...
        // Function to smoothly pan the map to a new center
        function smoothPanTo(map, newCenter, zoom) {
            var oldCenter = map.getCenter();
            var frames = 10; // Adjust as needed for smoothness (Reduced frames for smoother animation)
            var delay = 1; // Reduced delay for smoother animation

            var panStep = {
                lat: (newCenter.lat() - oldCenter.lat()) / frames,
                lng: (newCenter.lng() - oldCenter.lng()) / frames
            };

            function pan() {
                oldCenter = new google.maps.LatLng(
                    oldCenter.lat() + panStep.lat,
                    oldCenter.lng() + panStep.lng
                );
                map.panTo(oldCenter);

                if (--frames) {
                    setTimeout(pan, delay);
                } else {
                    // Conditionally set the final zoom level if provided
                    if (zoom !== undefined) {
                        map.setZoom(zoom);
                    }
                }
