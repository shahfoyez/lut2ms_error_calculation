                ...
                <div class="d-flex align-items-sm-center mb-7">
                    ...
                    <div class="d-flex align-items-center flex-row-fluid flex-wrap">
                        <div class="flex-grow-1 me-2">
                            <a href="{{ url('/employee/employeeEdit/'.$driver->id) }}" class="text-gray-800 text-hover-primary fs-6 fw-bolder">{{ $driver->name }}</a>
...
@section('mapScript')
    ...
    <script>
        const statusDiv = document.getElementById("map");
        if (!navigator.onLine) {
            statusDiv.innerHTML = "Map not available! You are currently offline.";
        } else {
            var map;
            var markers = [];
            var stoppageMarkers = [];
                        ...
                        if(wlsLength > 0 || wlhLength > 0){
                            ...
                            if(wlhLength > 0){
                                for (const route in withoutLocationHide) {
                                    ...
                                    buttonHtml = `<button class="foy_wlh_heading_button"><div class="foy_wlh_button_contents"><p class="foy_route_heading">Route: ${ route }</p></div></button>`;
                                    routeVehicles = withoutLocationHide[route];
                                    for(const vehicle in routeVehicles){
                                        ...
                                        buttonHtml += `<button class="foy_wlh_button"><div class="foy_wlh_button_contents"><p>${ busName }</p><p>Route: ${ tripRoute }</p><p>${ tripStart }</p></div></button>`;
                                    }
                                    routeDiv.innerHTML = buttonHtml;
                                    map.controls[google.maps.ControlPosition.TOP_LEFT].push(routeDiv);
                                }
                            }
                        }else{
                            noVehicle();
