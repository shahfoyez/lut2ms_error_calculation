            ...
            <div class="card mb-5 mb-xl-8">
                ...
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">Maintenance Records
                    ...
                    <div class="card-toolbar">
                        <?php
                            $formAction = url('/maintenance/maintenanceRecords/filter');
                            $refreshAction = url('/maintenance/maintenanceRecords');
                        ?>
                ...
                <div class="card-body py-3">
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="kt_datatable_example_5" class="table table-striped gy-5 gs-7 border rounded">
                            ...
                            <!--begin::Table body-->
                            <tbody>
                                @if($maintenanceRecords && $maintenanceRecords->count()>0)
                                    @foreach($maintenanceRecords as $entry)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <span class="text-muted fw-bold text-muted d-block fs-7">{{ $entry->from->format('h:i A') }}</span>
                                        </td>
                                        <td>
                                            @php
                                                if ($entry->to){
                                                    $date = $entry->to->format('d M Y');
                                                    $time = $entry->to->format('h:i A');
                                                    $toClass = "";
                                                }else{
                                                    $date = "No Data";
                                                    $time = "";
                                            ...
                                            @endphp
                                            <p href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6 {{ $toClass }}">{{ $date }}</p>
                                            <span class="text-muted fw-bold text-muted d-block fs-7">{{ $time }}</span>
                                        </td>
                                        <td>
                                            <p href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{ "TK: ".$entry->cost."/=" }}</p>
                                            {{-- <span class="text-muted fw-bold text-muted d-block fs-7">Paid</span> --}}
                                        </td>
                                        <td class="text-end">
                                            <a href="{{ url('/maintenance/edit/'.$entry->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                            ...
                                            <a href="#" data-bs-toggle="modal" data-bs-target="#delete_Modal" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm cd_modal" data-item="{{ url('/maintenance/delete/'.$entry->id) }}" >
